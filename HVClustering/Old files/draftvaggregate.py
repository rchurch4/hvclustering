...
#	- clusters through draft-like process (round robin)
...
g1 = cp.deepcopy(d1Teachers)
g2 = cp.deepcopy(d2Teachers)
g3 = cp.deepcopy(d3Teachers)
...
while len(clusters) < p:
	...
	while len(nextC) < k:
		#print "current cluster "+str(i)
		#print "next pick"
		if hasNoTeacher(nextC, 1):
			sizeofi = len(nextC)
			while sizeofi == len(nextC):
				blahblah = 0
				if len(d1Teachers) > 1:
					blahblah = rand.randint(0, len(d1Teachers)-1)
				nextPick = d1Teachers[blahblah]
				nextC.append(nextPick)
				d1Teachers.remove(nextPick)
		else: 
			if hasNoTeacher(nextC, 2):
				sizeofi = len(nextC)
				while sizeofi == len(nextC):
					blahblah = 0
					if len(d2Teachers) > 1:
						blahblah = rand.randint(0, len(d2Teachers)-1)
					nextPick = d2Teachers[blahblah]
					nextC.append(nextPick)
					d1Teachers.remove(nextPick)
			else: 
				if hasNoTeacher(nextC, 3):
					sizeofi = len(nextC)
					while sizeofi == len(nextC):
						blahblah = 0
						if len(d3Teachers) > 1:
							blahblah = rand.randint(0, len(d3Teachers)-1)
						nextPick = d3Teachers[blahblah]
						nextC.append(nextPick)
						d1Teachers.remove(nextPick)
				else:
					maxIdx = 0
